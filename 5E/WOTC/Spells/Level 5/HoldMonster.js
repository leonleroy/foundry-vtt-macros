...
let Maxtargets = (spellLevel <= 5) ? 1 : spellLevel - 4;
...
if (args[0].failedSaves.length > 0) {
    ...
    for (let target of targets) {
		...
		if ((find_target.actor.data.type === "npc") && ((find_target.actor.data.data.details.type.value.toLowerCase() === "humanoid") || ( (find_target.actor.data.data.details.type.value.toLowerCase() === "undead")) )) {
		console.log("Skipping humanoid or undead NPC: ",  find_target.actor.data.name);
		continue;
		}
        ...
        await find_target.actor.createEmbeddedDocuments("ActiveEffect", [Effect]);
    }
}